 ...
 */
function getCacheKey(wallet, txID, callbackUrl) {
 ...
 */
export async function wasSent(data) {
    if (!data.callbackUrl)
        return false;
 ...
 */
async function sendToCallback(data, callbackUrl) {
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            await cache.set(cacheKey, JSON.stringify({ confirmations: data.data.confirmations, attempts: 1, confirmed: true }), 3600);
        }
        else {
        ...
        console.error(`[Webhook] Error al enviar ${data.data.txID} => ${callbackUrl}:`, error.message);
        ...
        if (attempts <= MAX_RETRIES) {
            ...
            await cache.set(cacheKey, JSON.stringify({ confirmations: data.data.confirmations, attempts, confirmed: false }), 3600);
...
async function processRetryQueue() {
    ...
    while (retryQueue.length > 0) {
        const queuedItem = retryQueue.shift();
        if (!queuedItem.callbackUrl)
