...
class TronHelper {
    ...
    async getIncomingTransactionsFromTronGrid(walletAddress) {
        ...
        try {
            const response = await axios.get(`${this.tronGridBaseURL}/v1/accounts/${walletAddress}/transactions/trc20`, {
                headers: { 'TRON-PRO-API-KEY': apiKey },
   ...
   */
    async getCurrentBlock() {
        ...
        if (this.currentBlockCache.blockNumber &&
    ...
    async getTransactionInfo(txID) {
        ...
        const apiKey = await this.apiKeyTrongridManager.getAvailableKey();
        try {
            const response = await axios.post(`${this.tronGridBaseURL}/wallet/gettransactioninfobyid`, { value: txID }, // Asegúrate de enviar el JSON correcto
            {
                headers: {
   ...
   */
     ...
     * @returns {TransactionModel} Transacción normalizada.
     */
    async _transformTronGridTransaction(transaction) {
        let confirmations = 0;
        try {
            const currentBlockNumber = await this.getCurrentBlock(); // Obtener el bloque más reciente
            confirmations = Math.max(0, currentBlockNumber - transaction.block); // Calcular confirmaciones
